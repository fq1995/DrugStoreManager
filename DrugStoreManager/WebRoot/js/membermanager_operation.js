...
//			alert(param);
			if(confirm("您确定要删除吗？")){
				window.location.href="member_delMember.action?&ids="+param;
			}
		}
	}
	function update(){
		...
		if(count>1){
			...
			window.location.href="member_editMember.action?&id="+param;
		}
	}
	function init(){
		//查询
...
//			alert(keyword);
			window.location.href="member_showMember.action?currPage=1&keyword="+keyword;
		},false);
		//新增用户
		document.querySelector("#btn_addUser").addEventListener("click",()=>{
			window.location.href="member_doaddMember.action";
		},false);
		//修改用户
