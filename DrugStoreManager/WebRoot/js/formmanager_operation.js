...
//			alert(param);
			if(confirm("您确定要删除吗？")){
				window.location.href="form_delForm.action?&ids="+param;
			}
		}
	}
	function update(){
		...
		if(count>1){
			...
			window.location.href="form_editForm.action?&id="+param;
		}
	}
	function init(){
		//查询
...
//			alert(keyword);
			window.location.href="form_showForm.action?currPage=1&keyword="+keyword;
		},false);
		//新增
		document.querySelector("#btn_addUser").addEventListener("click",()=>{
			window.location.href="form_doaddForm.action";
		},false);
		//修改
