(function userManagerOperation(){
	function deleteUser(){
		...
		if(param==""){
			...
			alert(param);
			window.location.href="sale_delSale.action?&ids="+param;
		}
	}
	function update(){
		let checks=document.getElementsByName("id_check");
		...
		if(count>1){
			...
			window.location.href="sale_editSale.action?&id="+param;
		}
	}
	function init(){
		//查询
		document.querySelector("#btn_selectUser").addEventListener("click",()=>{
			...
			alert(keyword);
			window.location.href="sale_showSale.action?currPage=1&keyword="+keyword;
		},false);
		//新增用户
		document.querySelector("#btn_addUser").addEventListener("click",()=>{
			window.location.href="sale_doaddSale.action";
		},false);
		//修改用户
