...
//			alert(param);
			if(confirm("您确定要删除吗？")){
				window.location.href="buy_delBuy.action?&ids="+param;
			}
		}
	}
	function update(){
		...
		if(count>1){
			...
			window.location.href="buy_editBuy.action?&id="+param;
		}
	}
	function init(){
		//查询
...
//			alert(keyword);
			window.location.href="buy_showBuy.action?currPage=1&keyword="+keyword;
		},false);
		//新增用户
		document.querySelector("#btn_addUser").addEventListener("click",()=>{
			window.location.href="buy_doaddBuy.action";
		},false);
		//修改用户
		...
		document.querySelector("#btn_print").addEventListener("click",()=>{
			window.location.href="print_printUser.action";
		},false);
