...
<body>
	...
	<div class="rightinfo">
		...
		<div class="tools">
			...
			<ul class="toolbar1">
				<li style="border:0px"> <input class="form-control" placeholder="输入需要查询的药品剂型名" value="${requestScope.keyword }" style="width:180px;" type="text" id="keyword" name="keyword"/></li>&nbsp;&nbsp;
		...
		<table class="tablelist">
			...
			<tbody>
				<c:forEach items="${requestScope.page.list}" var="dosageform"
					varStatus="state">
					<tr>
						<td><input name="id_check" type="checkbox"
							value="${dosageform.dosageformId }" id="${dosageform.dosageformId}"/></td>
						<td>${state.count }</td>
						<td>${dosageform.dosageform }</td>
					</tr>
				</c:forEach>
			</tbody>
		</table>
			...
			<ul class="paginList">
				<c:if test="${page.perIndex > 0}">
					<li class="paginItem"><a
						href="${pageContext.request.contextPath }/form_showForm.action?currPage=1&keyword=${keyword}">首页</a></li>
					<li class="paginItem"><a
						href="${pageContext.request.contextPath }/form_showForm.action?currPage=${page.perIndex}&keyword=${keyword}"><span
				...
				<c:forEach begin="1" end="${page.totalPage }" var="p">
					<c:choose>
						...
						<c:otherwise>
							<li class="paginItem"><a
								href="${pageContext.request.contextPath }/form_showForm.action?currPage=${p}&keyword=${keyword}">${p}</a></li>
						</c:otherwise>
					</c:choose>
				</c:forEach>
				<c:if test="${page.nextIndex > 0}">
					<li class="paginItem"><a
						href="${pageContext.request.contextPath }/form_showForm.action?currPage=${page.nextIndex}&keyword=${keyword}"><span
							class="pagenxt"></span></a></li>
					<li class="paginItem"><a
						href="${pageContext.request.contextPath }/form_showForm.action?currPage=${page.totalPage}&keyword=${keyword}">尾页</a></li>
				</c:if>
				&nbsp;
				&nbsp;
...
window.location.href="${pageContext.request.contextPath}/form_showForm.action?keyword=${keyword}&currPage="+pc;
} 
</script>
</html>
